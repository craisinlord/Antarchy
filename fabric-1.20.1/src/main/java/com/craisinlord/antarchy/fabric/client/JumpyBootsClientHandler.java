...
public final class JumpyBootsClientHandler {
    ...
    public static void tick() {
        ...
        if (primeWindowTicks > 0) {
            primeWindowTicks--;
            if (jumpPressed) {
                com.craisinlord.antarchy.fabric.network.AntarchyFabricClientNetworking.sendToServer(new JumpyBootsLaunchPayload(storedChargeTicks, player.isSprinting()), JumpyBootsLaunchPayload.STREAM_CODEC, JumpyBootsLaunchPayload.TYPE);
                reset();
                return;
            }
            if (primeWindowTicks <= 0) {
        ...
        if (jumpPressed && wasCharging && chargeTicks > 0) {
            com.craisinlord.antarchy.fabric.network.AntarchyFabricClientNetworking.sendToServer(new JumpyBootsLaunchPayload(chargeTicks, player.isSprinting()), JumpyBootsLaunchPayload.STREAM_CODEC, JumpyBootsLaunchPayload.TYPE);
            reset();
            return;
        }
