...
public final class DreadClientHandler {
    private static final net.minecraft.world.effect.MobEffect DREAD_EFFECT = AntarchyFabricMisc.DREAD.get();
    ...
    public static void tick() {
        ...
        if (!player.hasEffect(DREAD_EFFECT)) {
            resetState();
            return;
        }
    ...
    public static void render(GuiGraphics guiGraphics) {
        Minecraft mc = Minecraft.getInstance();
        Player player = mc.player;
        if (player == null || !player.hasEffect(DREAD_EFFECT) || ACTIVE_APPARITIONS.isEmpty()) {
            return;
        }
