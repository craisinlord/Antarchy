...
public final class AntarchyFabricDispenserBehaviors {
    ...
    private static final DispenseItemBehavior SPAWN_EGG_BEHAVIOR = new DefaultDispenseItemBehavior() {
        ...
        public ItemStack execute(BlockSource source, ItemStack stack) {
            ...
            try {
                entityType.spawn(source.getLevel(), stack.getTag(), null, source.getPos().relative(direction), MobSpawnType.DISPENSER, direction != Direction.UP, false);
            } catch (Exception exception) {
                Antarchy.LOGGER.error("Error while dispensing spawn egg from dispenser at {}", source.getPos(), exception);
