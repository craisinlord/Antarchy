...
public final class HoverboardTrailSystem {
    ...
    public static void render(Camera camera, Matrix4f mat) {
        ...
        BufferBuilder.RenderedBuffer glowMesh = glowBuf.end();
        if (glowMesh != null) {
        ...
        BufferBuilder.RenderedBuffer coreMesh = coreBuf.end();
        if (coreMesh != null) {
    ...
    private static void buildPass(BufferBuilder buf, Matrix4f mat, List<TrailPoint> points, float[] color, Vec3 camPos, boolean glow) {
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            buf.vertex(mat, (float) (na.x + right.x), (float) (na.y + right.y), (float) (na.z + right.z)).color(color[0], color[1], color[2], alphaNearer).endVertex();
            buf.vertex(mat, (float) (na.x - right.x), (float) (na.y - right.y), (float) (na.z - right.z)).color(color[0], color[1], color[2], alphaNearer).endVertex();
            buf.vertex(mat, (float) (nb.x - right.x), (float) (nb.y - right.y), (float) (nb.z - right.z)).color(color[0], color[1], color[2], alphaFarther).endVertex();
            buf.vertex(mat, (float) (nb.x + right.x), (float) (nb.y + right.y), (float) (nb.z + right.z)).color(color[0], color[1], color[2], alphaFarther).endVertex();
        }
    }
