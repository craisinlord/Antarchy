...
public class GravityWalkNodeEvaluator extends WalkNodeEvaluator {
    ...
    public Node getStart() {
        ...
        node.costMalus = this.mob.getPathfindingMalus(BlockPathTypes.WALKABLE);
    ...
    public BlockPathTypes getBlockPathType(BlockGetter level, int x, int y, int z, Mob mob) {
        ...
        BlockPathTypes ceilingType = this.getBlockPathType(level, x, y + 1, z);
        if (mob.getPathfindingMalus(ceilingType) >= 0.0F) {
            return BlockPathTypes.OPEN;
        }

        BlockPathTypes result = BlockPathTypes.WALKABLE;
        for (int dw = 0; dw < this.entityWidth; dw++) {
            for (int dd = 0; dd < this.entityDepth; dd++) {
                for (int dh = 0; dh < this.entityHeight; dh++) {
                    BlockPathTypes type = this.getBlockPathType(level, x + dw, y - dh, z + dd);
    ...
    private int antarchy$addNeighbor(Node[] neighbors, int index, int x, int y, int z) {
        BlockPathTypes type = this.getBlockPathType(this.level, x, y, z, this.mob);
