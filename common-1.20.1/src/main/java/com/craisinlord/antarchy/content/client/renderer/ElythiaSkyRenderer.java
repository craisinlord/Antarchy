...
public final class ElythiaSkyRenderer {
        ...
        for (int i = 0; i < 1500; i++) {
            ...
            if (lenSq < 1.0D && lenSq > 0.01D) {
                ...
                for (int j = 0; j < 4; j++) {
                    ...
                    builder.vertex(sx + rx * size, sy + qx * size, sz + ry * size)
    ...
    private static void renderSunrise(PoseStack poseStack, Matrix4f projectionMatrix, ClientLevel level, float partialTick, float rainLevel) {
        ...
        buffer.vertex(matrix, 0.0F, 100.0F, 0.0F).color(sunriseColor[0], sunriseColor[1], sunriseColor[2], sunriseColor[3] * (1.0F - rainLevel)).endVertex();
        for (int i = 0; i <= 32; i++) {
            float angle = i * Mth.TWO_PI / 32.0F;
            float x = Mth.sin(angle) * 120.0F;
            float y = Mth.cos(angle) * 120.0F;
            buffer.vertex(matrix, x, y, -5.0F).color(sunriseColor[0], sunriseColor[1], sunriseColor[2], 0.0F).endVertex();
        }
    ...
    private static void renderBodyGlow(
        ...
        modelViewStack.mulPose(Axis.XP.rotation(orbitRad));
        modelViewStack.mulPose(Axis.ZP.rotation(yawRad));
        RenderSystem.applyModelViewMatrix();

        BufferBuilder buffer = Tesselator.getInstance().getBuilder();
        buffer.begin(VertexFormat.Mode.TRIANGLE_FAN, DefaultVertexFormat.POSITION_COLOR);
        buffer.vertex(0.0F, SKY_RADIUS, 0.0F).color(r, g, b, alpha).endVertex();
        for (int i = 0; i <= 16; i++) {
            float angle = i * Mth.TWO_PI / 16.0F;
            buffer.vertex(Mth.sin(angle) * glowRadius, SKY_RADIUS, Mth.cos(angle) * glowRadius).color(r, g, b, 0.0F).endVertex();
        }
        BufferUploader.drawWithShader(buffer.end());
    ...
    private static void renderTexturedBody(
        ...
        modelViewStack.setIdentity();
        modelViewStack.mulPoseMatrix(modelViewMatrix);
        modelViewStack.mulPose(Axis.XP.rotationDegrees(orbitDegrees));
        modelViewStack.mulPose(Axis.ZP.rotationDegrees(yawDegrees));
        modelViewStack.mulPose(Axis.YP.rotationDegrees(selfRotationDegrees));
        RenderSystem.applyModelViewMatrix();
        ...
        buffer.vertex(-halfSize, SKY_RADIUS, -halfSize).uv(minU, minV).endVertex();
        buffer.vertex(-halfSize, SKY_RADIUS, halfSize).uv(minU, maxV).endVertex();
        buffer.vertex(halfSize, SKY_RADIUS, halfSize).uv(maxU, maxV).endVertex();
        buffer.vertex(halfSize, SKY_RADIUS, -halfSize).uv(maxU, minV).endVertex();
        BufferUploader.drawWithShader(buffer.end());
