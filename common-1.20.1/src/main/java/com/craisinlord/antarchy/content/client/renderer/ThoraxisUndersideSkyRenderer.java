...
public final class ThoraxisUndersideSkyRenderer {
    ...
    private static final ResourceLocation MOON_TEXTURE = new ResourceLocation("minecraft", "textures/environment/moon_phases.png");
        ...
        for (int i = 0; i < 1800; i++) {
            ...
            if (lenSq < 1.0D && lenSq > 0.01D) {
                ...
                for (int j = 0; j < 4; j++) {
                    ...
                    builder.vertex((float) (sx + rx * size), (float) (sy + qx * size), (float) (sz + ry * size))
    ...
    private static void renderBodyGlow(Matrix4f modelViewMatrix, Matrix4f projectionMatrix, float orbitDegrees, float yawDegrees, int rgb, float glowRadius) {
        ...
        buffer.vertex(0.0F, SKY_RADIUS, 0.0F).color(red, green, blue, 0.55F).endVertex();
        for (int i = 0; i <= 24; i++) {
            float angle = i * Mth.TWO_PI / 24.0F;
            buffer.vertex(Mth.sin(angle) * glowRadius, SKY_RADIUS, Mth.cos(angle) * glowRadius).color(red, green, blue, 0.0F).endVertex();
        }
