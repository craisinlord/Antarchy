...
public final class DrTrayaurusTradeManager extends SimpleJsonResourceReloadListener {
    ...
    private static List<TradeEntry> defaultTrades() {
        return List.of(
                new TradeEntry(new StackEntry(new ResourceLocation("antarchy", "uranium_ingot"), 4), Optional.empty(), new StackEntry(new ResourceLocation("spyglass"), 1), 16, 1, 0.05F),
                new TradeEntry(new StackEntry(new ResourceLocation("antarchy", "titanium_ingot"), 6), Optional.empty(), new StackEntry(new ResourceLocation("antarchy", "duct_tape"), 2), 12, 5, 0.05F),
                new TradeEntry(new StackEntry(new ResourceLocation("antarchy", "uranium_ingot"), 8), Optional.of(new StackEntry(new ResourceLocation("antarchy", "kraken_tooth"), 1)), new StackEntry(new ResourceLocation("antarchy", "duplicator_sapling"), 1), 6, 10, 0.1F),
                new TradeEntry(new StackEntry(new ResourceLocation("antarchy", "titanium_ingot"), 10), Optional.empty(), new StackEntry(new ResourceLocation("antarchy", "easter_bunny_spawn_egg"), 1), 3, 5, 0.05F),
                new TradeEntry(new StackEntry(new ResourceLocation("antarchy", "uranium_ingot"), 16), Optional.of(new StackEntry(new ResourceLocation("antarchy", "titanium_ingot"), 8)), new StackEntry(new ResourceLocation("enchanted_golden_apple"), 1), 2, 20, 0.2F)
        );
    }
    ...
    private record TradeEntry(StackEntry buy, Optional<StackEntry> secondBuy, StackEntry sell, int maxUses, int villagerXp, float priceMultiplier) {
        ...
        private Optional<MerchantOffer> toOffer() {
            ...
            ItemStack buyStack = this.buy.toStack();
            if (buyStack.isEmpty()) {
                return Optional.empty();
            }

            ItemStack secondBuyStack = this.secondBuy.map(StackEntry::toStack).orElse(ItemStack.EMPTY);
