...
public class LucidAnchorBlockEntity extends BlockEntity {
    ...
    private static int colorForBeamState(BlockState state, int fallbackColor) {
        ...
        float[] diffuseComponents = beamBlock.getColor().getTextureDiffuseColors();
        int diffuse = ((int) (diffuseComponents[0] * 255.0F) << 16)
                | ((int) (diffuseComponents[1] * 255.0F) << 8)
                | (int) (diffuseComponents[2] * 255.0F);
