...
/** Shared deterministic layout used by Queen trail generation and locatequeen. */
public final class QueenTrailGrid {
    ...
    public static Site site(long worldSeed, int cellX, int cellZ) {
        ...
        return new Site(seed, x, z, Math.cos(angle), Math.sin(angle), 16 + random.nextInt(7), 6 + random.nextInt(2));
    }
    ...
    public record Site(long id, int terminalX, int terminalZ, double directionX, double directionZ, int footprints, int stride) {
        public int length() { return this.footprints * this.stride + 18; }
    }
}
