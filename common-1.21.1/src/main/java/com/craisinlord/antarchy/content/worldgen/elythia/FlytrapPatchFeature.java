...
public final class FlytrapPatchFeature extends Feature<NoneFeatureConfiguration> {
    private static final int PATCH_RADIUS = 5;
    private static final double PATCH_RADIUS_SQR = 30.25D;

    public FlytrapPatchFeature(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        FlytrapEntity flytrap = AntarchyObjects.FLYTRAP.get().create(serverLevel.getLevel());
        if (flytrap == null) {
            return false;
        }

        DifficultyInstance difficulty = serverLevel.getCurrentDifficultyAt(center);
        flytrap.moveTo(center.getX() + 0.5D, center.getY() + 1.0D, center.getZ() + 0.5D, random.nextFloat() * 360.0F, 0.0F);
        flytrap.finalizeSpawn(serverLevel, difficulty, MobSpawnType.CHUNK_GENERATION, null);
        flytrap.setPersistenceRequired();

        layPatch(level, center, random);
        serverLevel.addFreshEntity(flytrap);
        return true;
    }
    ...
    private static BlockState patchState(RandomSource random, int dx, int dz, double distanceSqr) {
        ...
        BlockState goo = AntarchyObjects.FLYTRAP_GOO_BLOCK.get().defaultBlockState();
