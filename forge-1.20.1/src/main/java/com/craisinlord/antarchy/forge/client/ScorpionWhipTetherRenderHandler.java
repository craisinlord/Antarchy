...
public final class ScorpionWhipTetherRenderHandler {
    ...
    public static void onRenderLevelStage(RenderLevelStageEvent event) {
        ...
        for (Map.Entry<Integer, Integer> entry : tethers.entrySet()) {
            ...
            buildTether(
                    buffer,
                    pose,
                    tetherStart(player, event.getPartialTick()),
    ...
    private static void buildTether(BufferBuilder buffer, Matrix4f pose, Vec3 start, Vec3 end, Vec3 cameraPos) {
        ...
        for (int i = 0; i < segments; i++) {
            ...
            buffer.vertex(pose, (float) (a.x + offset.x), (float) (a.y + offset.y), (float) (a.z + offset.z)).color(red, green, blue, 1.0F).endVertex();
            buffer.vertex(pose, (float) (a.x - offset.x), (float) (a.y - offset.y), (float) (a.z - offset.z)).color(red, green, blue, 1.0F).endVertex();
            buffer.vertex(pose, (float) (b.x - offset.x), (float) (b.y - offset.y), (float) (b.z - offset.z)).color(red, green, blue, 1.0F).endVertex();
            buffer.vertex(pose, (float) (b.x + offset.x), (float) (b.y + offset.y), (float) (b.z + offset.z)).color(red, green, blue, 1.0F).endVertex();
        }
    }
