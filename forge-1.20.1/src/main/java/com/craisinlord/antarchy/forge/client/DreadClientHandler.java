...
import com.craisinlord.antarchy.forge.registry.AntarchyForgeSounds;
...
public final class DreadClientHandler {
    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        if (!player.hasEffect(AntarchyForgeMisc.DREAD.get())) {
            resetState();
            return;
        }
    ...
    public static void render(GuiGraphics guiGraphics) {
        Minecraft mc = Minecraft.getInstance();
        Player player = mc.player;
        if (player == null || !player.hasEffect(AntarchyForgeMisc.DREAD.get()) || ACTIVE_APPARITIONS.isEmpty()) {
            return;
        }
    ...
    private static void spawnLungeFlash(Minecraft mc, Player player) {
        ...
        mc.level.playLocalSound(
                ...
                player.getY() + 1.0D,
                player.getZ(),
                AntarchyForgeSounds.NIGHTMARE_BITE.get(),
                SoundSource.HOSTILE,
                0.4F,
