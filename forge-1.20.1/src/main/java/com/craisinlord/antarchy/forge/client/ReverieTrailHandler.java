...
public final class ReverieTrailHandler {
    ...
    private static final class TrailPoint {
        ...
        float progress() {
            return lifetime <= 0 ? 1f : (float) age / (float) lifetime;
        }
    ...
    public static void onRenderLevelStage(RenderLevelStageEvent event) {
        ...
        BufferBuilder glowBuf = tesselator.getBuilder();
        glowBuf.begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_COLOR);
        for (List<TrailPoint> points : snapshots) {
            buildPass(glowBuf, mat, points, camPos, true);
        }
        BufferUploader.drawWithShader(glowBuf.end());

        BufferBuilder coreBuf = tesselator.getBuilder();
        coreBuf.begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_COLOR);
        for (List<TrailPoint> points : snapshots) {
            buildPass(coreBuf, mat, points, camPos, false);
        }
        BufferUploader.drawWithShader(coreBuf.end());
    ...
    private static void buildPass(BufferBuilder buf, Matrix4f mat, List<TrailPoint> points, Vec3 camPos, boolean glow) {
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            buf.vertex(mat, (float)(na.x + right.x), (float)(na.y + right.y), (float)(na.z + right.z)).color(cn[0], cn[1], cn[2], alphaNearer).endVertex();
            buf.vertex(mat, (float)(na.x - right.x), (float)(na.y - right.y), (float)(na.z - right.z)).color(cn[0], cn[1], cn[2], alphaNearer).endVertex();
            buf.vertex(mat, (float)(nb.x - right.x), (float)(nb.y - right.y), (float)(nb.z - right.z)).color(co[0], co[1], co[2], alphaFarther).endVertex();
            buf.vertex(mat, (float)(nb.x + right.x), (float)(nb.y + right.y), (float)(nb.z + right.z)).color(co[0], co[1], co[2], alphaFarther).endVertex();
        }
    }
